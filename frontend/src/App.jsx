...
function App() {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/tasks")
      ...
      .catch((error) => {
        setErro("Não será possível carregar as tarefas. ");
        setCarregando(false);
      });
  }, []);
      ...
      {carregando ? (
        ...
        <div className="quadro">
          <div className="coluna">
            <h2>A Fazer</h2>
            {tarefas
              .filter((tarefa) => tarefa.status === "a_fazer")
              .map((tarefa) => (
                <div key={tarefa.id} className="tarefa">
                  {tarefa.titulo}
                  <button onClick={() => moverTarefa(tarefa, "em_progresso")}>
                    Mover
                  </button>
          ...
          <div className="coluna">
            ...
            {tarefas
              ...
              .map((tarefa) => (
                <div key={tarefa.id} className="tarefa">
                  {tarefa.titulo}
                  <button onClick={() => moverTarefa(tarefa, "concluido")}>
                    Mover
                  </button>
                  <button onClick={() => excluirTarefa(tarefa.id)}>
                    Excluir
